...
from locations.test import room as test_room
...
from settings import get_uniform_scale
from utils import MENU_NATIVE_W, MENU_NATIVE_H, FONT_PATH, find_hovered
from dice import SkillCheck
from save_manager import ensure_initial_save, get_saved_location, load_rooms_state, save_game, SAVE_PLAYER_PATH
...
class Game:
    def __init__(self):
        ...
        ensure_initial_save(rooms)  #   save_manager -> загрузка последнего сейва игры 
        load_rooms_state(rooms)

        self.current_map = rooms[get_saved_location()] #   ставим последнюю локацию
    ...
    def _reload_player_from_save(self): #   Загружаемся из сейва
        load_rooms_state(rooms)
        self.current_map = self._get_saved_map()
        self.current_map.set_scale(get_uniform_scale(self.screen))
        ...
        for event in pygame.event.get():
            ...
            if self.death_active:
                ...
                if self.current_map.is_interactive(gx, gy): #   Если мы кликнули на интерактивный тайл
                    ...
                    if adjacent:    #   Когда придём чё делать будем?
